...
static float baro3041;             /* actual pressure */ 
static float temp3041;          /* actual temperature */
...
*******************************************************************************/  
static rt_err_t sm30_read_adc(rt_sensor_t psensor, float* p3041, float* t3041)
{
    rt_uint8_t buf30_read[4] = {0};
    rt_uint8_t status;
    rt_err_t ans;
    float dat = 0.0f;
    rt_int32_t baro_adc30;       /* pressure adc */
    rt_int32_t temp_adc30;    /* temperature adc */
    ...
    if(ans == RT_EOK)
    {
        ...
        if(status == 0)
        {
            baro_adc30 = ((buf30_read[0]&0x3f)<<8) | buf30_read[1];
            temp_adc30 = (buf30_read[2])<<3 | buf30_read[3]>>5;
       
            dat = ( psensor->info.range_max - psensor->info.range_min);
            *p3041 = (baro_adc30 - SM3041_MINCOUNT)* dat/ (SM3041_MAXCOUNT - SM3041_MINCOUNT) + psensor->info.range_min;
            *t3041 = (float)temp_adc30*200/2047 - 50;
            return RT_EOK;
        } 
    }
    return RT_ERROR;
...
static rt_size_t sm30_fetch_data(struct rt_sensor_device *psensor, void *buf, rt_size_t len )
{
    ...
    if(len > 0)
    {
        ...
        {           
            if(sm30_read_adc(psensor, &baro3041, &temp3041) == RT_EOK)
            {
                sensor_data->type = RT_SENSOR_CLASS_BARO;
                sensor_data->data.baro = baro3041 * SM30_BARO_X_N;
                sensor_data->timestamp = rt_sensor_get_ts();
                if(len > 1)
                {          
                    sensor_data++;
                    sensor_data->type = RT_SENSOR_CLASS_TEMP;                
                    sensor_data->data.temp = temp3041 * SM30_TEMP_X_N; 
                    sensor_data->timestamp = rt_sensor_get_ts();
                }             
            }
            else
